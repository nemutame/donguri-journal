...
コマンドを起動します（どの OS でも最も確実）。`npm ls -g donguri-journal` で確認できます。

> `donguri-journal` を直接実行して「動作確認」しないでください——これは stdio サーバーなので
> 入力待ちで止まるだけです。MCP クライアントから起動されるものです。
...
ローカル（stdio）サーバー `donguri-journal`、command `donguri-journal`（引数なし）を追加。
**実際に使うクライアントだけ**を編集し、既存の設定は保持してください。
...
完全に終了して開き直します。Claude Desktop は**タスクトレイから Quit**（ウィンドウを
閉じるだけでは不十分）。MCP サーバーは起動時に読み込まれます。
...
> （`npm view donguri-journal version`）を比較する。新しい版があれば「donguri-journal
> vX.Y.Z が出ています」と**短く**伝え、作業は中断しない。更新は**ユーザーが承認したとき
> だけ**: `npm i -g donguri-journal@latest` を実行し、MCP クライアントの再起動を案内する。
> オフラインや npm の失敗時は黙ってスキップする（次のセッションでまた試せばよい）。

重要な3原則: チェックは**セッションに一度**（呼び出しごとにしない）、**無断で更新しない**
（グローバルインストールはシステム状態の変更）、**オフラインでは静かに失敗する**。
...
解決しない場合は Issue へ: <https://github.com/nemutame/donguri-journal/issues>
