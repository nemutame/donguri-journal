...
最初に計画しているレンズは、Bullet Journal のデイリー/マンスリー/フューチャーログを
読み取り専用ツール（`bujo_day`, `bujo_month`, `bujo_future`, `bujo_reconcile`）で
描画します。データに BuJo の印は一切なく、すべての記号は導出されます:
...
完了、破棄、または**新しいエントリ**として今日へ持ち越し（body は書き直してよい。その摩擦
こそが本質）、`continues → 旧` でリンク。旧エントリの更新は不要で、incoming リンクの存在が
...
- **プラグイン基盤** — 契約＋kernel ctx ✅、ローカル導入＋動的ロード
  ...
  （§10）。🔜
- **レンズ層** — ビュー中立の注釈語彙＋ `entry_links` をコアに。その上に最初の読み取り専用
  射影として BuJo レンズ（デイリー/マンスリー/フューチャーログ＋マイグレーションの儀式）
  （§6）。🔜
