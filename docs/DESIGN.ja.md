...
7. **小さなコア、それ以外はすべて opt-in。** 重い/任意の機能（画像音声の便利機能・Eagle・
   ...
   第一の価値。
8. **コアはビュー中立。出力形式は「レンズ」。** 保存データに表示形式を焼き込まない（データの
   どこにも「BuJo」の印は現れない）。capture が記録するのは普遍的な意味論——性質 / 状態 /
   時間粒度 / エントリ間の相関——であり、Bullet Journal のような形式はその上の opt-in な
   **読み取り専用の射影**（§6）。レンズを外しても何も失われない。
9. **所有者は削除できる。** 原本は*システムによって*破壊されない（抽出/再インデックスで
   失わない）が、所有者は自分のデータを削除できる——誤って捕捉した秘密の**完全消去**を含む。
10. **言語は TypeScript。** 重い ML は LLM ＋埋め込みライブラリに委譲。本体は MCP ＋
...
向きは常に新→旧・追記のみ。ハード削除時は両方向のリンクも purge される。§6 参照。
...
境界で検証されます（それ以外の `meta` キーは従来通り自由）:
...
**新 → 旧**なので、関係づけが過去を書き換えることはありません。初期の `rel` 語彙:
`continues`（このエントリは過去のエントリの持ち越し/書き直し）と `references`（一般の関連）。

**書き換えの規律。** `body`・`occurred_at`・`content_hash`・ベクトルは不変。`meta` は
可変な*注釈*（状態変更は他に何も触れない——reindex 不要）。相関は追記のみ。
...
ファーストパーティのコードなのでインストールの儀式は不要）による opt-in で、有効化すると
...
`bujo_reconcile` が昨日の未裁定の open アクションを提示し、ユーザーは会話で一件ずつ裁く——
完了、破棄、または**新しいエントリ**として今日へ持ち越し（必要なら新規作成の時点で文面を
差し替える。その摩擦こそが本質）、`continues → 旧` でリンク。旧エントリの更新は不要で、incoming リンクの存在が
`>` として描画させます。日や月をまたぐ `continues` チェーンが*そのまま*マイグレーション
履歴です——「3回持ち越した。これは本当にやる価値がある？」がクエリ可能な事実になります。
...
エクスポートは意図的に「データを返すツール」にしない——§8 参照。
...
opt-in モジュールは `ctx.storage(moduleId)` 経由で、共有 SQLite ファイルに**自前のテーブル**を
作れる——レンズのカスタム・コレクション（所属・並び順）、ビュー設定、コネクタの同期状態など。
規則:

- **名前空間**: テーブルは `ext_<module>_*`（識別子を検証）。スキーマ版もモジュールごとに
  `schema_meta` の名前空間付きキーで管理し、コアのスキーマ版には決して触れない。
- **事実はコアに、構造はモジュールに。** ジャーナルの*事実*（想起に値するもの: タスク・
  出来事・メモ、その状態と相関）はコアの `entries` / `meta` / `entry_links` に置く——
  すべてのレンズ・エクスポート・将来の同期から見える場所に。モジュール・テーブルが持つのは
  **ビュー固有の構造**と**再構築可能なキャッシュ**であり、エントリは **id 参照のみ**。
- **エントリ内容のコピー禁止**——「秘密を確実に消せる」というハード削除の約束は拡張を
  越えて守られなければならない。`ctx.onEntryPurged(hook)` で参照の掃除を登録する: フックは
  purge の*トランザクション内*で走り、モジュール行とエントリは原子的に消える（フックが
  失敗すれば purge ごと中止——参照を残したまま消えるよりよい）。
- **コアテーブルの ALTER・直接書き込み禁止。** ジャーナルへの書き込みは必ず store の公開
  API 経由（dedup・埋め込み・リンク・tombstone を保つため）。プラグイン・セキュリティの
  他の部分と同じく、今日は契約＋レビュー、隔離は後段。
- **ライフサイクル**: アンインストール時、モジュール・テーブルは既定で残す（データは
  所有者のもの。求めに応じて purge）。DB ファイルごとのバックアップには自動で含まれる。
  JSONL エクスポートと Phase 2 の CRDT 同期はコアテーブルのみが対象——事実をコアに置く
  もう一つの理由。
- マニフェストは `storage` ケイパビリティを宣言し、インストール時の同意画面に出す。
